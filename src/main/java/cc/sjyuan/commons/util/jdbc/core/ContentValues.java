...
/**
 * This class is used to store a set of values that can process.
 */
public final class ContentValues {
			...
			if (value instanceof CharSequence) {
				try {
					...
					Logger.getLogger(TAG).log(
							Level.WARNING,
							"Cannot parse Long value for " + value + " at key "
									+ key);
					return null;
				}
			} else {
		...
		try {
			...
			if (value instanceof CharSequence) {
				try {
					...
					Logger.getLogger(TAG).log(
							Level.WARNING,
							"Cannot parse Integer value for " + value
									+ " at key " + key);
					return null;
				}
			} else {
		...
		try {
			...
			if (value instanceof CharSequence) {
				try {
					...
					Logger.getLogger(TAG).log(
							Level.WARNING,
							"Cannot parse Short value for " + value
									+ " at key " + key);
					return null;
				}
			} else {
		...
		try {
			...
			if (value instanceof CharSequence) {
				try {
					...
					Logger.getLogger(TAG).log(
							Level.WARNING,
							"Cannot parse Byte value for " + value + " at key "
									+ key);
					return null;
				}
			} else {
		...
		try {
			...
			if (value instanceof CharSequence) {
				try {
					...
					Logger.getLogger(TAG).log(
							Level.WARNING,
							"Cannot parse Double value for " + value
									+ " at key " + key);
					return null;
				}
			} else {
		...
		try {
			...
			if (value instanceof CharSequence) {
				try {
					...
					Logger.getLogger(TAG).log(
							Level.WARNING,
							"Cannot parse Float value for " + value
									+ " at key " + key);
					return null;
				}
			} else {
	...
	/**
	 * Judge the ContentValues whether it is empty
	 * 
	 * @param contentValues
	 * @return
	 */
	public static boolean isContentValuesEmpty(ContentValues contentValues) {
